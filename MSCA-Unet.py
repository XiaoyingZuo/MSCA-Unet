...
class U_Net():
    ...
    def load_data(self):
        ...
        for file in glob('./train/*'):  # 获取文件夹名称
            for filename in glob(file + '/*'):  # 获取文件夹中的文件
                ...
                x_train.append(img[128:384, 128:384])
        for file in glob('./label/*'):
            for filename in glob(file + '/*'):
                img = np.array(Image.open(filename), dtype='float32') / 255
                x_label.append(img[128:384, 128:384])
        ...
        # 图片，按9:1进行分配
        return x_train[:6211, :, :], x_label[:6211, :, :], x_train[6211:, :, :], x_label[6211:, :, :]
    ...
    def train(self, epochs=400, batch_size=4):
        ...
        fig.savefig('./evaluation/MSCA_Unet_curve.png', bbox_inches='tight',
                    pad_inches=0.1)  # 保存绘制曲线的图片
        plt.close()

    def test(self, batch_size=1):
        os.makedirs('./evaluation/test_result', exist_ok=True)
        os.makedirs("./evaluation/test_png", exist_ok=True)
        ...
        while index < test_num:
            ...
            if np.sum(mask) > 0 or np.sum(mask_true) > 0:
                ...
                path_test = 'test_result/png_' + \
                            str(index) + '.png'
                path_mask = 'test_result/mask_' + \
                    str(index) + '.png'
                path_pred = 'test_result/pred_' + \
                    str(index) + '.png'
            ...
            mask_true.save('./evaluation/test_result/' +
                           str(step) + 'true.png')
            index += batch_size
        acc = n / test_num
        dice = dice_sum / sum
